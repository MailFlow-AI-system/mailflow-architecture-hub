...
Normative source SHA-256: `dd551c46347bb408099cca11fb4fc22627070e476266da8924b40e6c213f2ecd`
...
MailFlow starts as a modular multi-tenant email system optimized for two developers and one to three pilot customers. Its Web application is independently deployed, while the `mailflow-core` Railway project runs separate API and worker services from the selected portable Dockerfile (pending implementation) over PostgreSQL, pg-boss, R2, Resend, and a planned private ClamAV workload. Railway provides isolated development, staging, and production environments and the project accepts the resulting MVP operating cost.

When Contact/Audience begins, the system crosses its first real distributed boundary: Audience becomes an independent service, RabbitMQ enters after its equivalence gate, Redis enters only for approved ephemeral workloads, a thin Gateway appears, database ownership becomes service-local, asynchronous integration uses outbox/inbox patterns, and application-level workload authorization remains mandatory. Audience may first run as a separate Railway service; VPS B and a private host network are future reference options when a later migration is justified. Later contexts are extracted only when domain ownership, scale, reliability, release cadence, or operational evidence justifies them.
...
| Backend             | One `mailflow-core` repository and selected portable Dockerfile/image recipe (pending implementation); separate Core API and Core worker processes.                                                                    |
...
| Compute             | Railway project with isolated development, staging, and production environments; paid VPS migration is future and evidence-triggered.                                                                                  |
| Containers          | Separate Railway Core API and worker services from the same Dockerfile; planned private ClamAV workload and OpenTelemetry Collector; no public worker ingress.                                                         |
...
| Future VPS A      | Edge, thin Gateway, Identity/Workspace, Mail API/worker, ClamAV, Collector, and later independently deployed Billing containers, if the self-managed migration is selected.              |
| Future VPS B      | Independent Audience API/worker and Collector. Campaign and Content may later be placed here only through the documented capacity gate.                                                  |
...
| Network           | Railway service networking for the MVP; a future self-managed topology may use provider-private networking, a selected private transport, provider/host firewalls, and app-level tokens. |
| Failure semantics | A future Audience placement failure degrades Audience only; it does not make Mail unavailable and does not provide Core high availability.                                               |
...
New services are placed on Railway or a future self-managed host only after a documented resource, latency, security, failure-domain, deployment, and cost review. Co-location never permits process, database, credential, migration, contract, or deployment coupling. If the selected placement has no safe headroom, another placement is provisioned.
...
Automation Runtime may start on a dedicated future VPS C or another independently operated placement. If self-managed hosts are selected, A, B, and C may form a direct private-transport mesh with no transit host. Runtime uses separate API, scheduler, and worker processes, its own database and identity, and direct authenticated access to owned dependencies. The complete topology's latency, capacity, routing, failure, and cost model is recalculated before activation; earlier measurements are not extrapolated.

## 5. Bounded-context catalog

| ID      | Context            | Owns                                                                                                                                                          | Initial deployable and placement                                                                                                        | Does not own                                                                                    |
| ------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| SVC-001 | Identity/Workspace | Better Auth integration, users, workspaces, memberships, invitations, teams, RBAC, tenancy context, session validation, and trust issuance                    | Core module on Railway in the MVP; extracted later only with evidence                                                                   | Frontend, domain resources of other services, or cross-service business authorization decisions |
| SVC-002 | Mail               | Mailboxes, inbound/outbound mail, threads, messages, drafts, attachments, inbox state, Resend adapter, delivery attempts, reconciliation, and provider policy | Core API/worker on Railway in the MVP; becomes Mail when Identity is extracted                                                          | Campaign orchestration, templates, or contact truth                                             |
| SVC-003 | Audience           | Contacts, lists, typed segment rules, tags, notes, deduplication, suppression audiences, imports/exports, snapshots, and audience health                      | First independent service; API/worker on Railway or a future VPS B reference placement                                                  | Campaign execution, provider sending, or direct access to Mail data                             |
| SVC-004 | Campaign           | Control owns definitions and launch intent; Execution owns batches, recipients, throttling, semantic retries, progress, and telemetry                         | One service with isolated modules, initially eligible for a future VPS B placement after its gate                                       | Resend credentials and technical provider reconciliation                                        |
...
| SVC-006 | Content            | Templates, reusable blocks, `EmailDocument`, variables, versions, compilation, validation, brand/localization assets, and immutable render artifacts          | One API/compiler-worker service, initially eligible for a future VPS B placement after its gate                                         | Puck runtime types, React elements, campaign execution, or provider delivery                    |
| SVC-007 | Workflow           | Graph drafts, revisions, validation, publication, immutable versions, runtime-plan compilation, templates, and control-plane state                            | Independent API/worker service; placement evaluated before implementation, with future VPS B only if it passes capacity/isolation gates | Workflow instance execution, delays, retries, provider actions, or contact journeys             |
| SVC-008 | Automation Runtime | Triggers, instances, transitions, delays, branching, joins, concurrency, retries, recovery, DLQ, action orchestration, and journey state                      | Future independently operated placement, potentially VPS C; API, scheduler, and worker processes                                        | Visual graph authoring or another service's provider/domain rules                               |
...
| SVC-011 | Billing            | Stripe integration, plan versions, subscription projection, entitlements, quotas, trials, invoices, and lifecycle policy                                      | Independent API/worker service on Railway or co-located on future VPS A initially                                                       | Product-domain authorization enforcement or direct payment-card storage                         |
...
| AUTH-015 | Host vs workload trust | A future private host transport such as WireGuard may authenticate hosts and encrypt links; audience-bound app tokens and target authorization remain mandatory in every deployment.      |
...
| Attachment scanning         | ClamAV                                                                                                                                        | Confirmed mandatory private MVP scanner behind a replaceable port; the planned Railway workload has no public ingress, while local-host deployment is a future VPS option.                                                                                                                                   |
| Secrets                     | Infisical Cloud with Railway Secret Sync                                                                                                      | Confirmed source of truth; `DATABASE_URL` syncs from `/mailflow-core` in each environment through three environment-scoped Railway Project Token connections and six service syncs. `APP_ENV` remains ordinary Railway configuration. Future independently permissioned runtimes receive their own identity. |
| Telemetry                   | OpenTelemetry, Pino, Collector, Grafana Cloud, Faro                                                                                           | Confirmed initial observability stack.                                                                                                                                                                                                                                                                       |
| Infrastructure              | Railway native deployment, Dockerfile, GitHub Actions CI; OpenTofu, cloud-init, Ansible, and Docker Compose retained for future VPS migration | Confirmed with non-overlapping ownership. Railway owns current CD; GitHub Actions owns CI.                                                                                                                                                                                                                   |
| Compute                     | Railway for MVP; future paid VPS migration                                                                                                    | Confirmed MVP platform with accepted operating cost. OCI and DigitalOcean remain historical future candidates, with provider selection deferred until migration evidence.                                                                                                                                    |
...
| OPS-005 | MVP compute               | Railway project with isolated `development`, `staging`, and `production` environments; separate API/worker services; US East (Virginia) aligned with Neon; operating cost accepted.                                                                                               |
| OPS-006 | Future host portability   | Multi-architecture images, external durable state, optional Cloudflare origin routing, Compose per host, OpenTofu/Ansible, and deployment locks preserve a later VPS migration path.                                                                                              |
| OPS-007 | Future host IaC ownership | If VPS migration occurs, OpenTofu owns cloud resources; cloud-init first boot; Ansible repeatable host config; Compose container topology; Actions orchestration. Railway remains the current infrastructure control plane.                                                       |
| OPS-008 | Future OpenTofu state     | If VPS migration occurs, use a dedicated private R2 bucket, per-environment state/credentials, client-side AES-GCM, native lockfile, CI concurrency, encrypted timestamped snapshots, and a recovery spike.                                                                       |
| OPS-009 | MVP ingress               | Railway-generated public domains for API services; worker services have no public HTTP ingress. Cloudflare Tunnel and Caddy are future VPS options.                                                                                                                               |
| OPS-010 | Deployment                | Railway native auto-deploy with Wait for CI for all six services; Dockerfile is the selected portable build recipe. Same-digest promotion remains an open CI/CD decision.                                                                                                         |
...
| OPS-012 | CI/CD                     | GitHub Actions CI for lint, format, typecheck, tests, and build; Railway native CD after Wait for CI. Intended promotion is `development -> staging -> production` through deliberate PRs; production branch mapping and digest strategy remain unresolved.                       |
...
| OPS-014 | Secrets delivery          | MVP uses `MailFlow-AI` with Development (`dev`), Staging, and Production plus `/mailflow-core`, `/mailflow-web`, and `/mailflow-site`; three environment-scoped Railway connections feed six service syncs. Human, future CI OIDC, and future runtime identities remain distinct. |
| OPS-015 | Observability             | OpenTelemetry SDKs and a private or external Collector are planned MVP requirements; host-local Collectors are a future VPS option. Pino JSON logs, Grafana Cloud, Faro, and privacy/cardinality controls remain selected.                                                        |
...
- Infisical paid/alternative: additional independently permissioned runtime, direct homologation runtime, path RBAC, version/PITR, audit, availability requirements, or future VPS migration.
...
- Physical host placement for later services, including any future VPS A/B/C assignment.
